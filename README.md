# Hi, I'm Luis Reyes Caro
### Software Developer in Training | Backend & Fullstack Enthusiast

**Status:** Open To Work  
**Specialization:** Backend & Fullstack Development

---

## About Me
I am an **adaptable and detail-oriented Software Developer** transitioning into the tech sector with a solid foundation in modern engineering workflows. Through rigorous, intensive project-based training at **Riwi Academy**, I focus on building robust, scalable, and highly efficient server-side applications. 

Driven by a passion for architectural patterns and logical puzzle-solving, I thrive on translating complex business requirements into clean, maintainable, and production-ready code.

---

## Tech Stack & Ecosystem

### **Languages & Core Web**
* **Backend & Logic:** Python, JavaScript (ES6+)
* **Frontend Basics:** HTML5, CSS3

### **Runtimes & Frameworks**
* **Node.js:** Scalable server-side JavaScript execution and asynchronous runtime management.
* **FastAPI:** High-performance, modern Python web framework for building robust RESTful APIs.

### **Tools & DevOps**
* **Version Control:** Git, GitHub (Branching workflows, PR reviews)
* **IDE & Environment:** VS Code, Terminal environments

---

### Current Focus & Engineering Insights

```python
CURRENT_FOCUS = {
    "Domain": "Backend Logic, Event-Driven Runtimes & Clean Architecture",
    "Environments": ["FastAPI (Python)", "Node.js (JavaScript)"],
    "Methodologies": ["SOLID Principles", "Clean Code", "Git Workflows"],
    "Learning": ["RESTful API Design", "Asynchronous Programming", "Fullstack Integration"]
}
```

### Featured Active Project
* **ATM System Architecture:** Engineering a comprehensive backend simulation designed to handle complex data workflows, strict validation logic, and secure transaction state management.
* **Technical Milestones:** Implementing bulletproof backend validations, data persistence routines, and maintaining a decoupled modular layout.

### Professional Objective
To secure a **Junior Developer position** within a forward-thinking engineering team. I look forward to contributing immediately to backend pipelines, refining API integrations, and accelerating my professional growth alongside industry veterans.


## Beyond the Code
<details>
<summary><b> Click to expand soft skills & personal interests</b></summary>
<br>

* **Core Competencies:** Rapid technological knowledge acquisition, critical structural thinking, and cross-functional team collaboration.
* **Interests:** Outside of writing code and designing backend architectures, I maintain a deep, active interest in Japanese literature, cultural history, and animation subcultures.
</details>



## 🛠️ Technologies & Ecosystem

| Layer | Technologies & Tools |
| :--- | :--- |
| **Frontend** | ![HTML](https://skillicons.dev/icons?i=sass) ![CSS](https://skillicons.dev/?i=vite) ![JS](https://skillicons.dev/icons?i=js) ![Sass](https://skillicons.dev/icons?i=css) ![Vite](https://skillicons.dev/?i=html) |
| **Backend (Python)** | ![Python](https://skillicons.dev/icons?i=python) ![FastAPI](https://skillicons.dev/icons?i=fastapi) |
| **Backend (JS / TS)** | ![NodeJS](https://skillicons.dev/icons?i=node) ![TypeScript](https://skillicons.dev/icons?i=typescript) ![Express](https://skillicons.dev/icons?i=express) |

---

